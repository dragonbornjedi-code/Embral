...
func get_ability_effect(id: String, attacker: WispData, defender: WispData) -> Dictionary:
	...
	var atk_elem := str(ability.get("element", ""))
	var mult := crystal_system.get_multiplier(atk_elem, defender.element)
	var val := int(ability.get("dmg", 0) * mult)

	return {
		"type": "damage",
		"value": val,
		"text": crystal_system.get_advantage_text(atk_elem, defender.element)
	}
